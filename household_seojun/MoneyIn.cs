...
namespace household_seojun
{
    public partial class MoneyIn : Form
    {
        ...
        {
            string[] list = { "기본급", "성과급", "초과수당", "복리후생비", "기타" };
            cbtype.Items.AddRange(list);
            cbtype.SelectedIndex = 0;

        }

        public MoneyIn(string dtp, string money, string category, string memo)
        {
            InitializeComponent();
            dateTimePicker1.Value = DateTime.Parse(dtp);
            cbtype.SelectedIndex = cbtype.Items.IndexOf(category);
            tbMoneyIn.Text = money;
            tbMemo.Text = memo;
            setCombo();
        }
        ...
        private void btOk_Click(object sender, EventArgs e)
        {
            ...
            //금액에 콤마표시
            //int money = int.Parse(tbMoneyIn.Text.Replace(",", ""));
            //tbMoneyIn.Text = money.ToString("N0");
